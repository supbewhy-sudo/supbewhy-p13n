...
需要学 Custom Instructions、AGENTS.md 语法
...
把长期习惯放到 Custom Instructions 或全局 AGENTS.md
...
项目做了很久，之前确认过的决策还是会被忘掉



用项目 MEMORY.md 记录决策、踩坑和纠正





同一套 Prompt 复制了很多遍，却没变成稳定流程
...
按工作类型分层：长期习惯 → 全局；项目规则 → 项目；重复流程 → Skill



只配当前用得到的：不用 Projects 就不配；不做本地项目就不需要 AGENTS.md



先预览后修改：任何写入都先给预览，确认后才执行



可迁移：换设备时导出脱敏包，新设备一键导入
...
给 Codex 的，不只是个性化，还有更稳的工程习惯

P13N 的 Codex 配置吸收了基于 Andrej Karpathy 对 LLM 编码问题观察而整理的工程原则：





先处理真正影响方向的歧义 — 不让小模糊拖慢节奏，也不让大歧义偷偷变成错误假设



选择最简单、充分的实现 — 不添加未要求的功能，不为单次使用创建抽象



只修改需求涉及的内容 — 匹配现有风格，不重构没坏的东西



用明确的完成标准验证结果 — 把"修好了"转化为可检查的条件

这些原则会根据你的工作方式，放进全局或项目 AGENTS.md，而不是强制塞给每一个用户。



这套原则来自社区根据 Karpathy 对 LLM 编码问题的观察整理的工程实践，上游来源。
...
xhs-page-planner（小红书卡片规划工具）

如果你需要为 P13N 或其他产品制作小红书内容，可以使用 xhs-page-planner。
...
📋 卡片规划：根据营销策略生成 5-6 页小红书卡片规划



📝 自动生成笔记：完成卡片规划后，自动生成配套小红书笔记
...
与卡片内容保持一致（使用相同的策略和核心价值）



固定结构：痛点识别 → 核心方法 → 关键差异 → 实际效果 → 行动 → 边界
...
精简为先：删除过渡和铺垫，保持快速阅读节奏

使用示例：

使用 $xhs-page-planner-v2.5 规划 P13N 的小红书内容

材料：https://github.com/supbewhy-sudo/supbewhy-p13n
...
下载 [dist/supbewhy-p13n.zip](dist/supbewhy-p13n.zip)



解压后把 supbewhy-p13n/ 目录复制到 ${CODEX_HOME:-$HOME/.codex}/skills/
...
修改 Custom Instructions、AGENTS.md 等
