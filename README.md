...
```text
使用 $supbewhy-p13n 盘点这台电脑上可以安全迁移的 Codex 个性化。先给我看迁移预览，等我确认后再导出脱敏迁移包。不要复制凭据、聊天记录和缓存。
```

把导出的 ZIP 通过可信方式带到新设备。安装 P13N 后再发送：

```text
使用 $supbewhy-p13n 预览这个迁移包：<文件路径>。先检查文件校验、目标位置和配置冲突，等我确认后再导入。
```
...
P13N 不会自动同步设备。如果旧设备已经无法使用，又没有迁移包或其他备份，旧设备上的本地个性化配置就无法还原，只能重新配置。云端设置和已经提交到 Git 的项目配置，仍可按原来的方式恢复。
